# Bandit Nivel 7 --> Nivel 8

## Objetivo
El siguiente nivel nos da un archivo data.txt en el cual hay varias lineas de nombres estas adjuntas a un codigo al lado derecho, nos indican
tambien que la linea por la cual deberia de buscar es millionth 

## Comandos usados
ls 
cat
grep 

##Explicaciòn
Cuando hacemos ls en la carpeta de bandit7 observamos que hay un archivo llamado data.txt el cual contiene muchas lineas de texto por lo que simplemente
filtramos lo que necesitamos con el comando "grep millionth data.txt" el cual nos da la contraseña al lado derecho de la palabra filtrada. 


## Contraseña para el siguiente nivel
morbNTDkSW6jIlUc0ymOdMaLnOlFVAaj
